...
def main():
    ...
    sample_file = res_path / f"mcmc_chip{chip_idx}_results_P=5hr.npz"
    ...
    # 4. Save and Plot
    np.save(res_path / f"posterior_mean_chip{chip_idx}_P=5hr.npy", mu_bar)
    np.save(res_path / f"posterior_var_chip{chip_idx}_P=5hr.npy", var_diag)
    
    i_mean_deg = np.rad2deg(np.arccos(np.mean(samples['cosi'])))
    plot_maps(np.asarray(mu_bar), np.sqrt(np.asarray(var_diag)), i_mean_deg, res_path, f"chip{chip_idx}_P=5hr")
